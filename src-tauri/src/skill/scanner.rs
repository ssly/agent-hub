...
pub fn scan_invalid_skills(
    skill_dir: &Path,
    platform_id: &str,
    platform_name: &str,
) -> Vec<InvalidSkill> {
    let mut invalid: Vec<InvalidSkill> = Vec::new();
    scan_invalid_recursive(
        skill_dir,
        skill_dir,
        platform_id,
        platform_name,
...
fn scan_invalid_recursive(
    current_dir: &Path,
    root_dir: &Path,
    platform_id: &str,
    platform_name: &str,
    invalid: &mut Vec<InvalidSkill>,
) {
    let Ok(entries) = fs::read_dir(current_dir) else {
        return;
    };
    ...
    for entry in entries.filter_map(|e| e.ok()) {
        ...
        if path.join("SKILL.md").exists() {
            ...
            let has_files = fs::read_dir(&path)
                .map(|mut e| {
                    e.any(|f| {
                        f.map(|fe| fe.file_type().map(|ft| ft.is_file()).unwrap_or(false))
                            .unwrap_or(false)
                    })
                })
                .unwrap_or(false);
