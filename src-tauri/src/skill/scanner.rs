...
pub fn scan_skills(skill_dir: &Path, platform_id: &str) -> Vec<Skill> {
    ...
    scan_recursive(skill_dir, skill_dir, platform_id, &mut skills);
    skills.sort_by(|a, b| a.folder.cmp(&b.folder).then(a.name.cmp(&b.name)));
    skills
}

fn scan_recursive(current_dir: &Path, root_dir: &Path, platform_id: &str, skills: &mut Vec<Skill>) {
    ...
    for entry in entries.filter_map(|e| e.ok()) {
        ...
        if path.join("SKILL.md").exists() {
            ...
            scan_recursive(&path, root_dir, platform_id, skills);
        }
    }
}
