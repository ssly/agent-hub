...
pub fn parse_skill(skill_dir: &Path, platform_id: &str) -> Option<Skill> {
    ...
    if !skill_file.exists() { return None; }
    ...
    let name = metadata.get("name")
        .and_then(|v| v.as_str())
        .unwrap_or_else(|| skill_dir.file_name().and_then(|n| n.to_str()).unwrap_or("unknown"))
        .to_string();

    let version = metadata.get("version").and_then(|v| v.as_str()).map(String::from);
    ...
    let symlink_target = if is_symlink { fs::read_link(skill_dir).ok() } else { None };
...
fn parse_frontmatter(content: &str) -> (HashMap<String, serde_yaml::Value>, String) {
    ...
    if !trimmed.starts_with("---") { return (HashMap::new(), content.to_string()); }
    let rest = &trimmed[3..];
