...
fn extract_session_summary(
    ...
    for line in reader.lines().take(METADATA_HEAD_LINES) {
        ...
        if record_type == Some("user")
            ...
            && data.get("isMeta").and_then(|value| value.as_bool()) != Some(true)
        {
...
fn parse_claude_message_line(data: &Value) -> Option<SessionMessage> {
    ...
    // Meta records are system injections (caveats, command stdout), not words
    // the user actually typed or saw from the assistant.
...
mod tests {
    ...
    #[test]
    fn parse_message_line_skips_tool_result_user_records() {
        // Tool results arrive as "user" records whose content blocks are
        // tool_result — they must not render as user messages.
        let tool_result = serde_json::json!({
            "type": "user",
            "message": {"content": [{"type": "tool_result", "content": "drawer.vue:119: const modifyUrl = 'x'"}]},
            "timestamp": "2026-07-30T09:00:00Z"
        });
        assert!(parse_claude_message_line(&tool_result).is_none());
    }

    #[test]
    fn parse_message_line_keeps_only_text_blocks() {
        let assistant = serde_json::json!({
            "type": "assistant",
            "message": {"content": [
                {"type": "thinking", "thinking": "let me check the files"},
                {"type": "text", "text": "已更换模板"},
    ...
    #[test]
    fn parse_message_line_skips_meta_records() {
        let meta = serde_json::json!({
            "type": "user",
            "isMeta": true,
            "message": {"content": "Caveat: the messages below were generated by the system"},
            "timestamp": "2026-07-30T09:00:00Z"
        });
        assert!(parse_claude_message_line(&meta).is_none());
    }
